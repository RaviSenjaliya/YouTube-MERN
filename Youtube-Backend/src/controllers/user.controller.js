...
const getCurrentUser = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(new ApiResponse(200, req.user, "current user fetch suucessfully"));
});
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  await deleteUserCloudniary(oldAvatar.avatar);
  const avatar = await uploadOnCloudinary(avatarLocalPath);
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!oldCoverImage) {
    throw new ApiError(400, "old CoverImage File is missing");
  }
  ...
  await deleteUserCloudniary(oldCoverImage.coverImage);
  const coverImage = await uploadOnCloudinary(coverImageLocalPath);
  ...
  return res
    ...
    .json(
      new ApiResponse(
        ...
        "WatchHistroy fetched successfully"
      )
    );
});
